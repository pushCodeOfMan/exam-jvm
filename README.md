...
##运行方式
1.在/jvm-restful目录启动终端，运行mvn jetty:run命令启动服务。通过http://127.0.0.1:8080进入主页后可以使用默认张三的信息创建交易记录，可以进行三种查询。
2.如果不使用浏览器，也可以再启动一个终端到/jvm-restful，通过Test的方式进行模拟操作。
3.xml保存了测试的6条数据。
##明显问题
1.登录信息未做完善，完成部分数据功能
2.交互信息未做到数据库，xml的创建多次，会使项目占用内存过大
2.0功能接口实现较少  
